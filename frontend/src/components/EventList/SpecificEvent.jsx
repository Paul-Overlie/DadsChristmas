import { NavLink, useNavigate, useParams } from "react-router-dom"
import { MdKeyboardArrowLeft } from "react-icons/md";
import { useDispatch, useSelector } from "react-redux";
import { useEffect, useRef, useState } from "react";
import { deleteEvent, restoreEvent } from "../../store/event";
import { FaRegClock } from "react-icons/fa";
import { FaMoneyBill1Wave } from "react-icons/fa6";
import { FiMapPin } from "react-icons/fi";
import './Event.css'

export const SpecificEvent = () => {
    const navigate = useNavigate()
    const dispatch = useDispatch()    
    const {eventId} = useParams()
    // const events = useSelector(state => state.events.events)
    const event = useSelector(state => state.events.currEvent)
    const userId = useSelector(state => state.session.user?.id)
    const ulRef = useRef()
    const modal = document.querySelector(".deleteEventModal")

    
let sendDate = (req) => {
    if(event){
        let [startDate, startTime] = event.startDate.split(" ")
        let [endDate, endTime] = event.endDate.split(" ")
        if(req==="startDate"){return startDate}
        if(req==="startTime"){return startTime}
        if(req==="endDate"){return endDate}
        if(req==="endTime"){return endTime}
    }
}
    

    let [seeModal, setSeeModal] = useState(false)

    useEffect(() => {
        dispatch(restoreEvent(eventId))
    }, [dispatch, eventId])
    
    const eventImg = () => {
        if(event) {
            if(event.EventImages.length>0) {
                return event.EventImages[0].url
            }
        }
    }

    const groupImg = () => {
        if(event) {
            if(event.groupImages) {
                if(event.groupImages[0]) {
                    return event.groupImages[0].url
                }
            }
        }
    }

    const openModal = (e) => {
        e.stopPropagation()
        setSeeModal(true)
        modal?.showModal()
    }

    useEffect(() => {
        if(!seeModal) {return}

        const closeModal = (e) => {
          if (ulRef.current && !ulRef.current.contains(e.target)) {
            setSeeModal(false);
            modal.close()
          }
        };
    
        document.addEventListener('click', closeModal);
        return () => document.removeEventListener('click', closeModal);
      }, [modal, seeModal]);

      const onDelete = () => {
        // let removeIndex = events.findIndex(e => e.id === eventId)
        dispatch(deleteEvent(eventId))
        navigate("/groups/"+event.Group.id)
      }

    return <div className="SpecificEventContainer">
        <div className="SpecificEventFirst">
            <NavLink to={"/events"}><MdKeyboardArrowLeft />Events</NavLink>
            <div className="SpecificEventFirstTitle">{event?.name}</div>
            <div className="SpecificEventFirstName">Hosted by {event?.organizer.firstName} {event?.organizer.lastName}</div>
        </div>
        <div className="SpecificEventSecondOutside">
            <div className="SpecificEventSecondInside">
                <div className="SESFirst">
                <img className="SpecificEventSecondImg" src={eventImg() || "data:image/jpeg;base64,"}/>
                <div className="SESFirstRight">
                {/* groupInfoBox */}
                <div className="SESFirstGroupContainer">
                    <img src={groupImg() || "data:image/jpeg;base64,"}/>
                    <div className="SESFirstText">
                        <div className="SESFirstName">{event?.Group.name}</div>
                        <div className="SESFirstPrivate">{event?.Group.private ? 'Private' : 'Public'}</div>
                    </div>   
                </div>
                {/* eventInfoBox */}
            <div className="SESFirstEventContainer">
                <div className="SESFirstTime">
                    <FaRegClock />
                    <div className="SESFirstTimes">
                        <div>START {sendDate("startDate")} · {sendDate("startTime")}</div>
                        <div>END {sendDate("endDate")} · {sendDate("endTime")}</div>
                    </div>
                </div>
                <div className="SESFirstPrice">
                    <FaMoneyBill1Wave />
                    <div>{event?.price ? "$"+event.price : "FREE"}</div>
                </div>
                <div className="SESFirstPlace">
                    <FiMapPin />
                    <div>{event?.type}</div>
                    {
                userId === event?.organizer.id ?
                <div className="SESFirstButtons">
                    <button>Update</button>
                    <button onClick={openModal}>Delete</button>

                    {/* Delete Event Modal */}
                <dialog className="deleteEventModal">
                    <div className="DeleteEventContainer" ref={ulRef}>
                        <div className="DeleteEventConfirmation">Confirm Delete</div>
                        <div className="DeleteEventQuestion">Are you sure you want to remove this event?</div>
                        <button className="DeleteEventYes" onClick={onDelete}>Yes (Delete Event)</button>
                        <button className="DeleteEventNo" onClick={() => {setSeeModal(false), modal.close()}}>No (Keep Event)</button>
                    </div></dialog>

                </div> : null
            }
                </div>
                </div>
            </div>
            </div>
            <div className="SpecificEventThirdContainer">
                    <div className="SpecificEventThirdTitle">Description</div>
                    <div className="SpecificEventThirdDescription">{event?.description}</div>
            </div>
            {/* Creator buttons */}
        </div>
    </div>
</div>
}
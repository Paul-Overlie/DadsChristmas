import { useParams, NavLink, useNavigate } from "react-router-dom"
import { MdKeyboardArrowLeft } from "react-icons/md";
import { deleteGroup, restoreGroup } from "../../store/groups";
import { useEffect, useRef, useState } from "react";
import { useDispatch, useSelector } from "react-redux";
import './GroupList.css'



export const SpecificGroup = () => {
    const {groupId} = useParams()
    const dispatch = useDispatch()
    const navigate = useNavigate()
    const ulRef = useRef()
    const button = useRef()
    const modal = document.querySelector(".deleteGroupModal")
    
    let [seeModal, setSeeModal] = useState(false)

    let currGroup = useSelector(state => state.groups.currGroup)
    let userId = useSelector(state => state.session.user?.id)

    if(currGroup?.Organizer?.id===userId){
        localStorage.setItem("groupId", JSON.stringify(currGroup))
    }
    

    let imgSrc = () => {
        if(currGroup){
            if(currGroup.GroupImages){
                return currGroup.GroupImages[0].url
            }
        }
    }

    //ordering events
    let eventList = []
    let beforeEvent = []
    let afterEvent = []

    currGroup?.events?.forEach(event => new Date()>new Date(event.startDate) ?
    afterEvent.push(event) : beforeEvent.push(event))

    beforeEvent.sort((a,b) => new Date(a.startDate)-new Date(b.startDate))

    eventList = [...beforeEvent, ...afterEvent]

    const createEvent = () => {
        navigate('/events/create')
    }

    const updateEvent = () => {
        navigate(`/groups/${groupId}/edit`)
    }

    useEffect(() => {
        dispatch(restoreGroup(groupId))
    }, [dispatch, groupId])

    const openModal = (e) => {
        e.stopPropagation()
        setSeeModal(true)
        modal.showModal()
    }

    useEffect(() => {
        if(!seeModal) {return}
        const closeModal = (e) => {
          if ((ulRef.current && !ulRef.current.contains(e.target))) {
            setSeeModal(false);
            modal.close()
          }
        };
    
        document.addEventListener('click', closeModal);
        return () => document.removeEventListener('click', closeModal);
      }, [modal, seeModal]);

      const onDelete = () => {
        dispatch(deleteGroup(groupId))
        navigate("/groups")
      }

    return <div className="SpecificGroupContainer">
        <div className="SpecificGroupTopContainer">
            <NavLink to={"/groups"}><MdKeyboardArrowLeft />Groups</NavLink>
            <img src = {imgSrc() || "data:image/jpeg;base64,"} />
            <div className="SpecificGroupTopText">
                <div className="SpecificGroupTitle">{currGroup?.name}</div>
                <div className="SpecificGroupTexts">{currGroup?.city}, {currGroup?.state}</div>
                <div className="SpecificGroupTexts">{currGroup?.events?.length} events · {currGroup?.private ? "Private" : "Public"}</div>
                <div className="SpecificGroupTexts">Organized by {currGroup?.Organizer?.firstName} {currGroup?.Organizer?.lastName}</div>
                {currGroup?.Organizer?.id===userId ? 
                <div className="SpecificGroupOwnerButtons">
                    <button onClick={createEvent} className="SpecificGroupOwnerButton">Create event</button>
                    <button onClick={updateEvent} className="SpecificGroupOwnerButton">Update</button>
                    <button onClick={openModal} className="SpecificGroupOwnerButton">Delete</button>

                    {/* Delete Group Modal */}
                    <dialog className="deleteGroupModal">
                        <div ref={ulRef} className="DeleteGroupContainer">
                            <div className="DeleteGroupConfirmation">Confirm Delete</div>
                            <div className="DeleteGroupQuestion">Are you sure you want to remove this group?</div>
                            <button className="DeleteGroupYes" onClick={onDelete}>Yes (Delete Group)</button>
                            <button ref={button} className="DeleteGroupNo" onClick={() => {setSeeModal(false), modal.close()}}>No (Keep Group)</button>
                        </div></dialog>
                </div>
                : userId ? <button className="SpecificGroupUserButton" onClick={() => alert("Feature coming soon")}>Join this group</button> : null}
            </div>
        </div>
        <div className="SpecificGroupBottomContainerContainer">
            <div className="SpecificGroupBottomContainer">
                <div className="SpecificGroupBottomOrganizer">Organizer</div>
                <div className="SpecificGroupBottomName">{currGroup?.Organizer?.firstName} {currGroup?.Organizer?.lastName}</div>
                <div className="SpecificGroupBottomAbout">{"What we're about"}</div>
                <div className="SpecificGroupBottomDescription">{currGroup?.about}</div>
                <div className="SpecificGroupBottomEventsTitle">Events ({currGroup?.events?.length})</div>
                {eventList?.map(event => {
                    let [startDate, startTime] = event.startDate.split(" ")
                    return <NavLink to={"/events/"+event.id} key={event.id}>
                        <div className="NavTop">
                            <img src={event.previewImage} />
                            <div className="NavTopRight">
                                <div className="NavTopDate">{startDate} · {startTime}</div>
                                <div className="NavTopName">{event.name}</div>
                                <div className="NavTopPlace">{event.Venue.city}, {event.Venue.state}</div>
                            </div>
                        </div>
                        <div className="NavBottom">{event.description}</div>
                    </NavLink> 
                })}
            </div>
        </div>
    </div>
}